...
```
T类型满足了定义在第一个片段的接口I，类型T可以传递给任何接受I作为参数的方法（源代码如下）：
```golang
type I interface {
    M() string
}
...
接口提供的灵活性来自于一个类型可以实现多个接口（看下面的代码）：
```golang
type I1 interface {
    M1()
}
...
## 静态类型vs动态类型
变量在编译阶段具有已知的类型，他是在生命时指定的，并且不会改变我们认为该变量是静态类型。接口类型本身也有静态类型，同时也具有动态类型，因此他们的赋值（源码如下）：
```golang
type I interface {
    M()
}
...
```
静态类型变量i是I类型的，它不会改变。另一方面，董涛类型就是动态的。第一次赋值后，i的动态类型是T1.但他不是固定不变的，因此第二次赋值后，i的类型变为了T2。当接口类型变量的值是nil（接口的空值），动态类型不会被设定

## 如何获取接口类型值的动态类型？
反射`(reflect)`包可以被用于实现这个（源码）：
...
## 行为的抽象
接口类型的值只允许访问接口类型提供的方法，隐藏了所有关于确切值的所有细节，想数据结构、数组、标量等等。（源码）：
```golang
type I interface {
    M1()
}
